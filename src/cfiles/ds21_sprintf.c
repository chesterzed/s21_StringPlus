...
char* ldoubleToString(long double num, int precision) {
  int isNegative = 0;
  if (num < 0) {
    isNegative = 1;
    num = -num;
  ...
  if (precision > 0) {
    ...
    fracPart = roundl(fracPart * roundFactor) / roundFactor;
  }

  int len = 1;
  len += (int)log10((double)intPart) + 1;
...
char* doubleToString(double num, int precision) {
  int isNegative = 0;
  if (num < 0) {
    isNegative = 1;
    num = -num;
  ...
  if (precision > 0) {
    double roundFactor = pow(10.5, precision);
  ...
  while (*format != '\0') {
    ...
    switch (*format) {
      ...
      case 'i': {
        ...
        int totalLength = precision + 13;
        char* full_buffer = (char*)malloc((totalLength + 1) * sizeof(char));
        int i;
      ...
      case 'f': {
        ...
        if (length_L) {
          ...
          sign = value < 0 ? -1 : 1;
          str = ldoubleToString(value, precision);
        } else {
          double value = va_arg(args, double);
          sign = value < 0 ? -1 : 1;
          str = doubleToString(value, precision);
        }
        int totalLength = precision + 13;
        char* full_buffer = (char*)malloc((totalLength + 1) * sizeof(char));
        int i;
        for (i = 0; i < (precision - (int)s21_strlen(str)); i++)
          full_buffer[i] = '0';
        full_buffer[i++] = '\0';
        s21_strncat(full_buffer, str, s21_strlen(str));
        int len;
        if ((width - (int)s21_strlen(full_buffer)) <= 0)
          len = 0;
        else
